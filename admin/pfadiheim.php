<?php
/* 
 * Setting up responsible person for pfadiheim
 */

function add_pfadiheim_meta()
{
    global $post;
    if(!empty($post))
    {
        if($post->post_name == 'pfadiheim' )
        {
            add_meta_box(
                'pfadiheim_meta', // $id
                'Verantwortlich', // $title
                'select_responsible_person', // $callback
                'page', // $page
                'normal', // $context
                'high'); // $priority
        }
...
add_action('add_meta_boxes', 'add_pfadiheim_meta');

function select_responsible_person()
{
	//get meta
	global $post;
	...
	function save_responsible_person($post_id, $post){
		if($post->post_name == 'pfadiheim') {
	...
	add_action('save_post','save_responsible_person',1,2);
?>